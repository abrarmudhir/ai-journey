...
from wqulibs.ab_test.params import cohort_params
...
class StudentGenerator:
    ...
    def generate_students(self):
        ...
        # Create name
        df["firstName"] = df["gender"].apply(lambda x: names.get_first_name(gender=x))
        ...
        if self.is_experiment:
            ...
            df["group"] = pd.array([pd.NA] * len(df), dtype="string")
            # Get parameters
            response_var = self.cohort_params["experiment"]["response_var"]
            contingency_table = self.cohort_params["experiment"]["contingency_table"]
